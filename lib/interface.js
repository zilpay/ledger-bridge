...
const { ZilliqaMessage } = require('@zilliqa-js/proto/dist/index')
...
export default class Zilliqa {
  ...
  signHash(keyIndex, message) {
    ...
    let msgBuffer = Buffer.from(
      ZilliqaMessage.ProtoTransactionCoreInfo.encode(serialised).finish()
    )

    const STREAM_LEN = 128 // Stream in batches of STREAM_LEN bytes each.
    let txn1Bytes
    if (msgBuffer.length > STREAM_LEN) {
      msgBuffer = msgBuffer.slice(0, STREAM_LEN)
      msgBuffer = msgBuffer.slice(STREAM_LEN, undefined)
    } else {
      txn1Bytes = msgBuffer
      msgBuffer = Buffer.alloc(0)
    }

    const txn1SizeBytes = Buffer.alloc(4)
    txn1SizeBytes.writeInt32LE(txn1Bytes.length)
    const hostBytesLeftBytes = Buffer.alloc(4)
    hostBytesLeftBytes.writeInt32LE(msgBuffer.length)
    // See signTxn.c:handleSignTxn() for sequence details of payload.
    // 1. 4 bytes for indexBytes.
    // 2. 4 bytes for hostBytesLeftBytes.
    // 3. 4 bytes for txn1SizeBytes (number of bytes being sent now).
    ...
    return transport
      ...
      //     let txnNBytes
      //     if (msgBuffer.length > STREAM_LEN) {
      //       txnNBytes = msgBuffer.slice(0, STREAM_LEN)
      //       msgBuffer = msgBuffer.slice(STREAM_LEN, undefined)
      //     } else {
      //       txnNBytes = msgBuffer
      //       msgBuffer = Buffer.alloc(0)
      ...
      //       txnNBytes
      //     ])
      //     return transport.send(CLA, INS.signHash, P1, P2, payload).then(cb)
      //   }
      //   return response
      // })
      .then((result) => {
